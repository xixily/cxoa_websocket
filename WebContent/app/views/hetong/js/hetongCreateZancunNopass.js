...
/*------------------------------------------------------发票-----------------------------------------------------------*/
	...
	$("#sureAboutFapiaoSale").attr("style","display:inline;");
	$("#updateFapiaoSale").attr("style","display:none;");
...
//添加发票
$("#sureAboutFapiaoSale").click(function(){
	...
	var data ={"hetongNumber":hetongNumber,"applicationDate":applicationDate,"money":money,"capitalMoney":capitalMoney,"company":company,"departMement":departMement,"type":type,"name":name,"date":date,"remark":remark,"yujihuikuanDate":yujihuikuanDate};
	  $.post('public/ht/addFaPiao.action',data,function(res){
			if(res.success==true){
				...
				"<td>"+res.obj.money+"</td><td>"+a+"</td><td>"+b+"</td><td>"+res.obj.remark+"</td><td><a id="+res.obj.id+" class='dele' href='javascript:;' onclick='updateFapiao2("+res.obj.id+",this)'>修改</a></td></tr>";	
...
//更新发票(回显)
function updateFapiao2(id,obj){
	tr = $(obj).parent().parent();
	
	//确定按钮隐藏    修改删除按钮显示
	$("#sureAboutFapiaoSale").attr("style","display:none");
	$("#updateFapiaoSale").attr("style","display:inline");
...
return false;
	...
	//修改发票
	$("#updateFapiaoSale").click(function(){
		$("#kaipiaoAmountError").text("");
...
$(function(){
	...
	$("#saleSubmitAgain").click(function(){
	    ...
	    var data={"id":id,"company":company,"depart":depart,"cid":cid,"didNum":didNum,"contractMoney":contractMoney,"agreementNumber":agreementNumber,"endTime":endTime,"agreementText":agreementText,"remarksText":remarksText,"payMethod":payMethod,"state":state}		
	     ...
	     //保存按钮
		$("#contractSaveAgain").click(function(){
			var flag = true;
			var b = validateContractAmount();
			if(b==false){
				flag = false;
